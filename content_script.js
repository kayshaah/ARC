...
(function () {
  ...
  // -------------- make floating panel once
  function makePanelOnce() {
    if (document.getElementById("rt-panel-host")) return;

    const host = document.createElement("div");
    host.id = "rt-panel-host";
    host.style.position = "fixed";
    host.style.right = "12px";
    host.style.bottom = "12px";
    host.style.zIndex = 2147483646; // on top
  ...
  // -------------- attach a badge to each review
  function attachBadges() {
    const reviews = findReviewNodes();
    reviews.forEach(node => {
      if (node.querySelector(".rt-badge-host")) return; // already has one
      ...
      // host
      const host = document.createElement("div");
      host.className = "rt-badge-host";
      host.style.position = "relative";
      node.style.position = node.style.position || "relative";
      node.prepend(host);

      // shadow for the badge (so Amazon CSS can’t break it)
      ...
      // compute demo score now and paint badge text/color
      ...
      // clicking badge updates the floating panel with reasons
      badge.addEventListener("click", () => {
        const panelHost = document.getElementById("rt-panel-host");
        if (!panelHost) return;
        ...
        content.innerHTML = `
          ...
          <div class="hint">Demo only. We’ll add real ML later.</div>
        `;
      });
    });
  }

  // -------------- watch for more reviews (amazon loads dynamically)
